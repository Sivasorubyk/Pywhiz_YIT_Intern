...
const CodePage7 = () => {
  ...
  const [code, setCode] = useState(`# Milestone 7: Error Handling
# Create a function that safely divides two numbers

# 1. Write a function called 'safe_divide' that takes two parameters (a and b)
# and returns a/b. If b is 0, it should return "Cannot divide by zero" instead
# of raising an error.

# Your code here:


# 2. Create a function called 'open_file' that takes a filename as parameter
# and safely tries to open and read the file. If the file doesn't exist, it
# should return "File not found" instead of raising an error.
...
`)
  ...
  // Hardcoded lesson data for milestone 7
  const lessonData = {
    title: "Milestone 7: Error Handling in Python",
    description:
      "In this exercise, you'll practice error handling in Python using try-except blocks. You'll create functions that can gracefully handle errors rather than crashing when something unexpected happens.",
  }
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      // Check if the code is correct (contains try-except blocks)
      const isCorrect = 
        code.includes("def safe_divide") && 
        code.includes("def open_file") && 
        code.includes("try:") && 
            ...
            <div className="bg-white rounded-xl p-6 shadow-md flex-grow">
              ...
              {isSuccess && (
                <div className="mb-4 p-3 bg-green-50 border border-green-200 text-green-600 rounded-lg flex items-start">
                  ...
                  <span>Excellent! Your code properly handles potential errors using try-except blocks.</span>
                </div>
              )}
