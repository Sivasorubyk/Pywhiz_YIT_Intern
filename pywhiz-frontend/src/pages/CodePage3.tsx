...
const CodePage3 = () => {
  ...
  const [code, setCode] = useState(`# Milestone 3: Loops
# Create a for loop to print the first 5 numbers in the Fibonacci sequence
# Hint: The Fibonacci sequence starts with 0, 1 and each subsequent number is 
...
`)
  ...
  // Hardcoded lesson data for milestone 3
  const lessonData = {
    title: "Milestone 3: Loops in Python",
    description:
      "In this exercise, you'll practice creating and using loops in Python. Loops allow you to execute a block of code multiple times. You'll use a for loop to generate the Fibonacci sequence, where each number is the sum of the two preceding ones.",
  }
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      // Check if the code is correct (produces the Fibonacci sequence)
      const isCorrect = 
        (output && output.includes("0") && output.includes("1") && output.includes("2") && output.includes("3")) || 
        code.includes("for") || 
        code.includes("while")
            ...
            <div className="bg-white rounded-xl p-6 shadow-md flex-grow">
              ...
              {isSuccess && (
                <div className="mb-4 p-3 bg-green-50 border border-green-200 text-green-600 rounded-lg flex items-start">
                  ...
                  <span>Great job! You've successfully created a loop to generate the Fibonacci sequence.</span>
                </div>
              )}
