...
const CodePage5 = () => {
  ...
  const [code, setCode] = useState(`# Milestone 5: Conditionals and Decision Making
# Create a program that determines the grade based on a score

# Write a function called 'get_grade' that takes a score (0-100) as input
# and returns the appropriate letter grade according to this scale:
# 90-100: A
# 80-89: B
# 70-79: C
# 60-69: D
...
scores = [95, 82, 78, 65, 55]

# Print the grade for each score, like "Score 95 gets an A"
# Your code here:
...
`)
  ...
  // Hardcoded lesson data for milestone 5
  const lessonData = {
    title: "Milestone 5: Conditionals in Python",
    description:
      "In this exercise, you'll practice using conditional statements (if, elif, else) to make decisions in your code. You'll create a grading program that assigns letter grades based on numerical scores.",
  }
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      // Check if the code is correct (contains conditionals and produces correct output)
      const isCorrect = 
        code.includes("def get_grade") && 
        code.includes("if") && 
        (code.includes("elif") || code.includes("else if") || code.includes("else")) && 
        (output && output.includes("Score 95 gets an A") && output.includes("Score 55 gets an F"))
            ...
            <div className="bg-white rounded-xl p-6 shadow-md flex-grow">
              ...
              {isSuccess && (
                <div className="mb-4 p-3 bg-green-50 border border-green-200 text-green-600 rounded-lg flex items-start">
                  ...
                  <span>Great job! Your grading program correctly uses conditionals to assign the right grades.</span>
                </div>
              )}
